...
class STOLMC_Service_Tracker_Api_Users extends STOLMC_Service_Tracker_Api {
	 ...
	 *
	 * Uses a database transaction to ensure atomicity: either the user
	 * and all meta data are created, or neither are.
	 *
	 * @since 1.5.0 Added transaction support for atomic creation.
	 *
	 * @param WP_REST_Request $data The REST request object.
	 *
	...
	public function create( WP_REST_Request $data ): WP_REST_Response {
		...
		$transaction = new \STOLMC_Service_Tracker\includes\Utils\STOLMC_Service_Tracker_WordPress_Transaction();
		...
		try {
			...
			if ( is_wp_error( $user_id ) ) {
				...
				/**
				 * Fires when user creation fails.
				 *
				 * @since 1.5.0
				 *
				 ...
				 * @param object          $body      The raw request body.
				 */
				do_action( 'stolmc_service_tracker_user_create_failed', $user_id, $user_data, $body );
			...
			if ( ! empty( $body->phone ) ) {
				$phone_result = update_user_meta( $user_id, 'phone', sanitize_text_field( $body->phone ) );
				if ( $phone_result !== false ) {
					$meta_data['phone'] = sanitize_text_field( $body->phone );
				} else {
					$meta_errors[] = 'Failed to save phone number';
				}
			}

			// Store cellphone as user meta if provided.
			if ( ! empty( $body->cellphone ) ) {
				$cellphone_result = update_user_meta( $user_id, 'cellphone', sanitize_text_field( $body->cellphone ) );
				if ( $cellphone_result !== false ) {
					$meta_data['cellphone'] = sanitize_text_field( $body->cellphone );
				} else {
					$meta_errors[] = 'Failed to save cellphone number';
				}
				}

			// Check if any meta operations failed
			if ( ! empty( $meta_errors ) ) {
				$transaction->rollback();
				
				/**
				 * Fires when user meta creation fails.
				 *
				 * @since 1.5.0
				 *
				 * @param int             $user_id     The user ID that was created.
				 * @param array           $meta_errors The meta operation errors.
				 * @param array           $user_data   The user data.
				 * @param object          $body        The raw request body.
				 */
				do_action( 'stolmc_service_tracker_user_meta_create_failed', $user_id, $meta_errors, $user_data, $body );

				return $this->rest_response(
					[
						'success' => false,
						'message' => 'User created but failed to save meta data: ' . implode( ', ', $meta_errors ),
						'transaction' => 'rolled back',
					],
					500
				);
			...
			if ( false === $user ) {
				// This shouldn't happen since we just created the user, but handle it
				return $this->rest_response(
					[
						'success' => false,
			...
			/**
			 * Fires when user creation throws an exception.
			 *
			 * @since 1.5.0
			 *
			 ...
			 * @param object          $body      The raw request body.
			 */
			do_action( 'stolmc_service_tracker_user_create_exception', $e, $user_data, $body );

			// Log the error for debugging
			error_log( 'Service Tracker: User create transaction failed - ' . $e->getMessage() );
